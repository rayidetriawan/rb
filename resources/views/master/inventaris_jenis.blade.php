...
@section('content')
        ...
        <div class="modal-dialog">
            <div class="modal-content border-0">
                <div class="modal-header p-3 bg-primary" data-v-01bddeea="">
                    <h5 class="modal-title text-light" id="exampleModalLabel" data-v-01bddeea=""> Tambah Jenis Inventaris </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"
                        id="close-modal" data-v-01bddeea=""></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('inventaris.jenissimpan') }}" method="POST">
                        @csrf
                        <div class="row g-3">
                            <div class="col-xxl-12">
                                <div>
                                    <label for="grup"
                                        class="form-label @error('grup') text-danger @enderror">Group Inventaris <i class="text-danger fs-15">*</i></label>
                                    <select data-width="100%" class="form-control" name="grup" id="pilihgrup" required>
                                        <option value="">Pilih Group Inventaris</option>
                                        @foreach($grup as $dt)
                                            <option value="{{ $dt->group_id }}">{{ $dt->group_name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mt-2">
                                    <label for="validationCustom02"
                                        class="form-label @error('jenis') text-danger @enderror">Nama Jenis Inventaris <i class="text-danger fs-15">*</i></label>
                                    <input type="text" name="jenis" oninput="this.value = this.value.toUpperCase()" required
    ...
    <div class="modal fade zoomIn" id="modaledit" tabindex="-1" aria-labelledby="exampleModalgridLabel" data-v-01bddeea=""
        ...
        <div class="modal-dialog">
            <div class="modal-content border-0">
                <div class="modal-header p-3 bg-primary" data-v-01bddeea="">
                    <h5 class="modal-title text-light" id="exampleModalgridLabel">Edit Jenis Inventaris</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"
                        id="close-modal" data-v-01bddeea=""></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('inventaris.jenisupdate') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id" id="id" value="{{ old('id') }}">
                        <input type="hidden" name="grupold" id="grupold" value="{{ old('grupold') }}">
                        <div class="row g-3">
                            ...
                            <div class="col-xxl-12">
                                ...
                                <div>
                                    <label for="grup"
                                        class="form-label @error('grup') text-danger @enderror">Group Inventaris <i class="text-danger fs-15">*</i></label>
                                    <select data-width="100%" class="form-control" name="grup" id="pilihgrup2" required>
                                        <option value="">Pilih Group Inventaris</option>
                                        @foreach($grup as $dt)
                                            <option value="{{ $dt->group_id }}">{{ $dt->group_name }}</option>
                                        @endforeach
                                ...
                                <div class="mt-2">
                                    <label for="validationCustom02"
                                        class="form-label @error('edit_nama') text-danger @enderror">Nama Jenis Inventaris</label>
        ...
        <div class="col-xl-9">
            <div div class="card">
                <div class="card-header" >
                    <h6 class="card-title mb-0 font-poppins fs-15" >Master Jenis Inventaris</h6>
                </div>
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" data-v-cd5f1dea=""
                                                        ...
                                                        <li data-v-cd5f1dea="">
                                                            <a href="javascript:void(0)" class="dropdown-item edit"
                                                                ...
                                                                data-id="{{ $item->jenis_id }}" data-grup="{{ $item->group_id }}"><i
                                                                    class="ri-pencil-fill align-bottom me-2 text-muted"
                                                                    data-v-cd5f1dea=""></i> Edit</a>
                                                        </li>
                                                        <li data-v-cd5f1dea="">
                                                            <a class="dropdown-item remove-item-btn confirm-delete"
                                                                data-v-cd5f1dea="" data-id="{{ $item->jenis_id }}">
                                                                <i class="ri-delete-bin-fill align-bottom me-2 text-muted"
                                                                    data-v-cd5f1dea="">
                                                                </i>
                                                                Delete
                                                                <form method="POST"
                                                                    action="{{ route('inventaris.groupjenis', ['id' =>$item->jenis_id, 'grup' =>$item->group_id]) }}"
                                                                    id="delete-{{ $item->jenis_id }}">
                                                                    @csrf
                                                                    @method('delete')
                                                                </form>
                                                            </a>
...
@push('scripts')
    <script>
        $(document).ready(function () {
            $("#pilihgrup").select2({
                theme: "bootstrap",
                placeholder: "Pilih Grup Inventaris",
                dropdownParent: $('#exampleModalgrid .modal-content')
            });
            $("#pilihgrup2").select2({
                theme: "bootstrap",
                placeholder: "Pilih Grup Inventaris",
    ...
    <script>
        $(document).on('click', '.edit', function() {
            ...
            $.get(id + '/' + grup + '/jenisedit', function(data) {
                //success data
                console.log(data);
                $('#id').val(data.jenis_id);
                $('#grupold').val(data.group_id);
                $('#pilihgrup2').val(data.group_id).trigger('change');
                $('#edit_nama').val(data.nama_jenis);
