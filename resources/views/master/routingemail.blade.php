...
@section('title', 'Master Data Routing Email')
@section('nav_active_routing_email', 'active')
        ...
        <div class="modal-dialog">
            <div class="modal-content border-0">
                ...
                <div class="modal-body">
                    <form action="{{ route('routing_email.simpan') }}" method="POST">
                        @csrf
                        <div class="row g-3">
                            ...
                            <div class="col-xl-12">
                                <div>
                                    <label for="validationCustom04"
                                        class="form-label @error('nama') text-danger @enderror">User <i class="text-danger">*</i></label>
                                    <select data-width="100%" class="form-control" name="user" id="pilihuser" required>
                                        <option value="">Pilih User</option>
                                        @foreach($user as $usr)
                                            <option value="{{ $usr->nik }}" >{{ $usr->jabatan['nama_jabatan'] }} {{ $usr->departemen['nama_dept'] }} | {{ $usr->nama }}</option>
                                        @endforeach
                                    </select>
                                    @error('nama')
                            ...
                            <div class="col-xl-12">
                                <div>
                                    <label for="validationCustom04"
                                        class="form-label @error('cabang') text-danger @enderror">Cabang <i class="text-danger">*</i></label>
                                    
                                    <select data-width="100%" class="form-control" name="cabang" id="pilihcabang" required>
                                        <option value="">Pilih Cabang</option>
                                        @foreach($cabang as $bnk)
                                            <option value="{{ $bnk->branch_id }}-{{ $bnk->schema_name }}" >{{ $bnk->schema_name }} | {{ $bnk->branch_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-xl-12">
                                <div>
                                    <label for="validationCustom04"
                                        class="form-label @error('nama') text-danger @enderror">Grup <i class="text-danger">*</i></label>
                                    <select data-width="100%" class="form-control" name="grup" id="pilihgrup" required>
    ...
    <div class="modal fade zoomIn" id="modaledit" tabindex="-1" aria-labelledby="exampleModalgridLabel" data-v-01bddeea=""
        ...
        <div class="modal-dialog">
            <div class="modal-content border-0">
                ...
                <div class="modal-body">
                    <form action="{{ route('routing_email.update') }}" method="POST">
                        @csrf
                        ...
                        <div class="row g-3">
                            <div class="col-xl-12">
                                <div>
                                    <label for="validationCustom04"
                                        class="form-label @error('nama') text-danger @enderror">User <i class="text-danger">*</i></label>
                                    <select data-width="100%" class="form-control" name="user" id="pilihuser2" required>
                                        <option value="">Pilih User</option>
                                        @foreach($user as $usr)
                                            <option value="{{ $usr->nik }}" >{{ $usr->jabatan['nama_jabatan'] }} {{ $usr->departemen['nama_dept'] }} | {{ $usr->nama }}</option>
                                        @endforeach
                            ...
                            <div class="col-xl-12">
                                <div>
                                    ...
                                    <select data-width="100%" class="form-control" name="cabang" id="pilihcabang2" disabled required>
                                        <option value="">Pilih Cabang</option>
                                        @foreach($cabang as $bnk)
                                            <option value="{{ $bnk->branch_id }}-{{ $bnk->schema_name }}" >{{ $bnk->schema_name }} | {{ $bnk->branch_name }}</option>
                                        @endforeach
                            ...
                            <div class="col-xl-12">
                                <div>
                                    <label for="validationCustom04"
                                        class="form-label @error('nama') text-danger @enderror">Grup <i class="text-danger">*</i></label>
                                    <select data-width="100%" class="form-control" name="grup" id="pilihgrup2" disabled required>
                                        <option value="">Pilih Grup</option>
                                            <option value="Ahass" >Ahass</option>
                                            <option value="Dealer" >Dealer</option>
                                    </select>
        ...
        <div class="col-xl-12">
            <div div class="card">
                <div class="card-header" >
                    <h6 class="card-title mb-0 font-poppins fs-15" >Master Routing Email</h6>
                </div>
                <div class="card-header border-0" data-v-cd5f1dea="">
                    <div class="row g-4" data-v-cd5f1dea="">
                        ...
                        <div class="col-12 col-md-6" data-v-cd5f1dea="">
                            <form action="{{ route('routing_email.cari') }}" method="get">
                            <div class="d-flex justify-content-md-end justify-content-center" data-v-cd5f1dea="">
                                <div class="search-box ms-2" data-v-cd5f1dea=""><input type="text"
                                        class="form-control" value="{{ old('search') }}" placeholder="Cari User.." data-v-cd5f1dea=""
                        ...
                        <table class="table align-middle table-nowrap" id="customerTable">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th data-sort="email">Nama</th>
                                    <th data-sort="date">Email</th>
                                    <th data-sort="phone">Departemen</th>
                                    <th data-sort="date">Schema</th>
                                    <th data-sort="date">Cabang</th>
                                    <th data-sort="date">Grup</th>
                                ...
                                @if(count($data) > 0)
                                    @foreach ($data as $no => $item)
                                        <tr>
                                            ...
                                            <td class="email">{{ $item->departement['nama_dept'] }}</td>
                                            <td class="email">{{ $item->schema_name }}</td>
                                            <td class="email">{{ $item->cabang['branch_name'] }}</td>
                                            <td class="email">{{ $item->flag }}</td>
                                            <td class="date">@if(!empty($item->update_date)) @tanggal($item->update_date) @else @endif</td>
                                            <td>
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end" data-v-cd5f1dea=""
                                                            ...
                                                            <li data-v-cd5f1dea="">
                                                                <a class="dropdown-item remove-item-btn confirm-delete"
                                                                    ...
                                                                    Delete
                                                                    <form method="POST"
                                                                        action="{{ route('routing_email.hapus', $item->id) }}"
                                                                        id="delete-{{ $item->id }}">
                                                                        @csrf
                                                                        @method('delete')
                                                                    </form>
...
@push('scripts')
    ...
    <script>
        $(document).on('click', '.edit', function() {
            var id = $(this).data('id');
            var url = "{{ route('routing_email.edit', ':id') }}";
            $.get(url.replace(':id', id), function(data) {
                //success data
                // console.log(data);
                var cabang = data.branch_id+'-'+data.schema_name;
                console.log(cabang);
                $('#id').val(data.id);
                $('#pilihcabang2').val(cabang).trigger('change');
                $('#pilihuser2').val(data.nik).trigger('change');
                $('#pilihgrup2').val(data.flag).trigger('change');
