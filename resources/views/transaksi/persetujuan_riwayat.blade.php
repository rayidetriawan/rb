                            ...
                            <div class="col-sm-4">
                                <div class="search-box">
                                    <input type="text" class="form-control search"
                                        placeholder="Pencarian untuk Rb Id, Item.." id="search" name="search" autocomplete="off" onkeydown="return (event.keyCode!=13);">
                            ...
                            <table class="table table-nowrap align-middle" id="orderTable">
                                <thead class="text-muted table-light">
                                    <tr class="text-uppercase">
                                        ...
                                        <th class="sort desc" data-sort="id">Item</th>
                                    ...
                                    @foreach ($data as $no => $hist)
                                        <tr>
                                            ...
                                            <td class="id align-top">
                                                <a href="{{ route('pengajuan.detail',$hist->RB_ID) }}">{{ $hist->RB_ID }} <i class="ri-link"></i></a>
                                                <br><div class="text-muted">Branch : 
                                                {{ $hist->cabang->branch_name }}</div>
                                            </td>
                                            <td class="id align-top">
                                                <ul class="ps-2 vstack mb-1">
                                                    @foreach($hist->TrxRbDetailItem as $item)
                                                        <li>{{ $item->item }}
                                                            <div class="text-muted"> {{ $item->QTY }} x Rp.&nbsp; {{ $item->HARGA }} = Rp. {{ $item->QTY*$item->HARGA }}
                                                            </div>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                                <br>Total = Rp.&nbsp;{{ $hist->TOTAL_HARGA }}    
                                            </td>
                                            <td class="align-top">
                                                {{ $hist->karyawan['nama'] }}
                                            </td>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $('#search').on('keyup', function() {
            ...
            $.ajax({
                type: 'get',
                url: '{{ URL::to('persetujuan/liveSearch') }}',
                data: {
                    'search': $value,
                    'cabang'    : $cabang,
                    'status'    : $status,
